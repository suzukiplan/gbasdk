...
int main(int argc, char* argv[])
{
    ...
    rc++;
    if (argc < 3) {
        fprintf(stderr, "usage: bmp2pal input.bmp palette.dat\n");
        goto ENDPROC;
    }
    ...
    /* パレットを RGB555 形式に変換 */
    for (int i = 0; i < 256; i++) {
        unsigned short r = (pal256[i] & 0x00F80000) >> 19;
        unsigned short g = (pal256[i] & 0x0000F800) >> 6;
        unsigned short b = (pal256[i] & 0x000000F8) << 7;
        palgba[i] = r | g | b;
    }

    /* GBA の Palette RAM 形式でファイルを書き出す */
    rc++;
    ...
    // BG
    fwrite(palgba, 1, sizeof(palgba), fpW);
    // Sprite (BGと同じパレット)
    fwrite(palgba, 1, sizeof(palgba), fpW);
